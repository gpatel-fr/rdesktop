...
typedef struct _DEVICE_FNS
{
	...
	RD_NTSTATUS(*read) (RD_NTHANDLE handle, uint8 * data, uint32 length, unsigned long offset,
			    uint32 * result);
	RD_NTSTATUS(*write) (RD_NTHANDLE handle, uint8 * data, uint32 length, unsigned long offset,
			     uint32 * result);
